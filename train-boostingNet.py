...
def main():
    ...
    for n in range(5,boosting_number):
        ...
        trainset = boosting_data.ISBIDataset(train_df, training_img_path, weight_path, testing=False)
        valset = boosting_data.ISBIDataset(val_df, evaluation_img_path, weight_csv = None, testing=True)
        ...
        checkpoint_callback = ModelCheckpoint(monitor='val_loss', 
                                            ...
                                            filename=f'ISBI-WeightedBCE-boosting-b1-ResNext101-b{n}'+'-{epoch:03d}-{val_loss:.4f}',
                                            save_top_k=3,
                                            mode='min')
        ...
        next_id = n+1
        weight_df.to_csv(f'boosting/weight_b{next_id}.csv', index=False)
        #print(weight)
        #print(count)
